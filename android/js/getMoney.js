...
//	alert("asd");
//	$.ajax({
//		url: getCookie("tempUrl") + '/Member/clock.do?token=' + getCookie("token") + "&phone=" + getCookie("phone"),
//		type: "POST",
	...
	$.ajax({
		...
		success: function(res) {
			if(res.code == 0) {
				...
				if(judge) {
					$.ajax({
						url: getCookie("tempUrl") + '/Withdrawal/application.do?token=' + getCookie("token") + "&money=" + money + "&alipayAccount=" + alipayAccount + "&memberId=" + getCookie("id"),
						type: "POST",
						datatype: "application/json",
						contentType: "application/json;charset=utf-8",
						success: function(res) {
