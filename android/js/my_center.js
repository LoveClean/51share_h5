...
$(function() {
	...
	$.ajax({
		...
		success: function(res) {
			if(res != null && res != "" && res != "null" && res.code == 0) {
				var obj = res.data;
				...
				$("#reward").html("奖励: ￥" + (obj.availableBalance + obj.frozenBalance).toFixed(2));
			}else{
...
//	var auths = sessionStorage.getItem("auth");
	...
	$.ajax({
		url: getCookie("tempUrl") + "/Member/loginout.do?token=" + getCookie("token"),
		type: "POST",
		success: function(res) {
