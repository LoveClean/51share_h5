...
$(function () {
    setCookie("jumpurl", window.location.href);
	sessionStorage.setItem("productview","productview1");
...
$(function(){
	...
	$.ajax({
        url:  getCookie("tempUrl")+'/commodity/getInfo.do?id='+id2+"&token="+getCookie("token"),
        type: "GET",
        	...
        	var data=res.data;
            	if (res.code== 0){
	                ...
	                $("#commodity_prices").text("￥"+data.commodityPrices);
	               	...
	               	$("#commodity_commission").text("预计奖励："+"￥"+(data.commission*data.commodityPrices).toFixed(2));
