//setCookie('tempUrl', "http://119.23.29.186:8096/");
setCookie('tempUrl', "http://47.106.189.124:8096/");

checkUserStatus(getCookie("id"));
...
$(function() {
	...
	setCookie("jumpurl", window.location.href, 30);
	...
	$.ajax({
		url: getCookie("tempUrl") + "/Image/getImage.do",
		type: "GET",
...
function toartilcle() {
	...
	$.ajax({
		url: getCookie("tempUrl") + "/Image/selectInfo.do",
		type: "POST",
...
$(function() {
	...
	$.ajax({
		url: getCookie("tempUrl") + "/article/getAllByStatus?pageNum=10&pageSize=3&token=" + getCookie("token"),
		type: "GET",
		success: function(result) {
...
//				<img src='http://share-app.oss-cn-qingdao.aliyuncs.com/2018052313286e693c-77f5-4e68-8a99-792962af4d7e.png?x-oss-process=image/resize,h_200'>
				...
				var yong = $("<div class='article_yong'></div>").append($("<span style='color:#F9DC86;'>奖励：￥"+item.commission+"</span>"));
				var share = $("<div class='icon_share'></div>").append($("<img src='css/images/rou.png' onclick='toshare(this)' articleId='" + item.id + "'><img src='css/images/share.png' onclick='toshare(this)' articleId='" + item.id + "' ><img src='css/images/in.png' onclick='toshare(this)' articleId='" + item.id + "' >"));
				
				var info = $("<div class='article_info index_article'></div>");
				var aaa = $("<a class='articleId' ></a>");
				info.appendTo(".article_list");
				aaa.append(icon).append(span).append(yong).append(share).appendTo(info);
...
$(function() {
	...
	$.ajax({
		url: getCookie("tempUrl") + "/commodity/getAllByStatus.do?token=" + getCookie("token"),
		type: "GET",
		success: function(result) {
			var list = result.data;
			$.each(list, function(index, item) {
				...
				var price = $("<div class='price'></div>").append($("<span>价格：<span style='color:#F9DC86;'>"
				+ item.commodityPrices +"</span></span><span>&nbsp / &nbsp奖励：<span style='color:#F9DC86;'>"
				+yong + "</span></span>"));
				
				it.append(hd).append(good_bottom.append(name).append(price)).appendTo("#prolist");
...
//			type: "GET",
//			success: function(res) {
//				setCookie("tologin","",30);
//				},
//			error: function(res) {
//				setCookie("tologin","tologin",30);
//			}
//		});
//});