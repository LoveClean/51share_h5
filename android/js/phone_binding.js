...
//	alert(phone);
	$.ajax({
		url: getCookie("tempUrl") + "/common/sms/sendCaptcha",
		type: "POST",
...
if(phone == "" || password2 == "" || phoneCode == "") {
	...
	$.ajax({
		url: getCookie("tempUrl") + "/Member/PhoneBinding.do",
		type: "POST",
		datatype: "application/json",
		contentType: "application/json;charset=utf-8",
		data: JSON.stringify({
		...
		success: function(result) {
			...
			setCookie('truename', sessionStorage.getItem("v1"), {path: '/'});
                setCookie('token', result.data.accessToken, {path: '/'});
                setCookie('id', sessionStorage.getItem("v3"), {path: '/'});
                setCookie('openId', sessionStorage.getItem("v4"), {path: '/'});
                setCookie('phone', result.data.phone, {path: '/'});
